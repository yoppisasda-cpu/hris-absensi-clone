...
export default function EditPurchaseOrderModal({ isOpen, onClose, onSuccess, po }: any) {
    ...
    useEffect(() => {
        if (isOpen && po) {
            setFormData({
                ...
                notes: po.notes || "",
                items: po.items?.length > 0 ? po.items.map((item: any) => ({
    ...
    const handleSubmit = async (e: any) => {
        ...
        setLoading(true);
        try {
            await api.put(`/inventory/purchase-orders/${po.id}`, formData);
            toast.success("Pesanan Pembelian (PO) berhasil diperbarui.");
            onSuccess();
            onClose();
            // Reset form
            setFormData({
            ...
            <div className="glass w-full max-w-4xl max-h-[95vh] rounded-[3rem] border border-white/10 relative overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300 flex flex-col">
                <div className="bg-slate-950/50 border-b border-indigo-500/20 px-10 py-8 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-sm font-black italic tracking-widest text-white uppercase leading-none">Edit Pipeline Pengadaan (PO)</h3>
                            <p className="text-[9px] font-bold text-slate-500 uppercase tracking-[0.2em] mt-2 italic">Perubahan Data Pesanan Pembelian</p>
                        </div>
                    </div>
                    ...
                    <div className="mt-10 flex gap-4 shrink-0">
                        ...
                        >
                            {loading ? (
                                ...
                                <><ShoppingBag className="h-4 w-4 stroke-[3px]" /> SIMPAN PERUBAHAN PO</>
                            )}
                        </button>
                    </div>
                </form>
