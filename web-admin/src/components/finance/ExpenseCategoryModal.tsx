...
export default function ExpenseCategoryModal({ isOpen, onClose, onSuccess }: any) {
    ...
    const handleAdd = async (e: React.FormEvent) => {
        ...
        setLoading(true);
        try {
            await api.post('/finance/expense-categories', { name: newCategoryName, type: 'OPERATIONAL' });
            setNewCategoryName("");
            fetchCategories();
            onSuccess();
    ...
    const handleUpdate = async (id: number) => {
        ...
        try {
            await api.patch(`/finance/expense-categories/${id}`, { name: editingName });
            setEditingId(null);
            fetchCategories();
            onSuccess();
            ...
            <div className="glass w-full max-w-md rounded-[3rem] border border-white/10 relative overflow-hidden shadow-2xl animate-in zoom-in-95 duration-300">
                <div className="bg-slate-950/50 border-b border-indigo-500/20 px-10 py-8 flex items-center justify-between">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <h3 className="text-sm font-black italic tracking-widest text-white uppercase leading-none">Kategori Pengeluaran</h3>
                ...
                <div className="p-10 space-y-8 max-h-[80vh] overflow-y-auto no-scrollbar">
                            ...
                            categories.map((cat) => (
                                <div key={cat.id} className="flex items-center justify-between p-5 rounded-[1.8rem] border border-white/5 bg-slate-950 group hover:border-indigo-500/30 transition-all shadow-inner">
                                    {editingId === cat.id ? (
                                        ...
                                        <>
                                            <div className="flex items-center gap-4">
                                                <div className="h-2 w-2 rounded-full bg-slate-800 group-hover:bg-rose-500 transition-colors shadow-[0_0_10px_rgba(244,63,94,0)] group-hover:shadow-[0_0_10px_rgba(244,63,94,0.5)]"></div>
                                                <div className="flex flex-col">
                                                    <span className="font-black text-white text-xs uppercase tracking-widest italic">{cat.name}</span>
                                                    <span className="text-[8px] font-black text-slate-500 tracking-[0.2em]">{cat.type}</span>
                                                </div>
                                            </div>
                    ...
                    <div className="flex items-start gap-4 p-6 rounded-[2rem] bg-rose-500/5 border border-rose-500/10">
                        <AlertCircle className="h-5 w-5 text-rose-400 shrink-0 mt-0.5" />
                        <p className="text-[9px] text-slate-600 font-bold leading-relaxed uppercase tracking-widest italic">
                            Pengelompokan kategori membantu Anda memantau arus kas keluar pada laporan laba rugi dengan lebih terperinci.
                        </p>
                    </div>
                </div>
            </div>
